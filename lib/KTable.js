...
class KTable extends StreamDSL {
    ...
    consumeUntilLatestOffset(finishedCallback = null){
        console.warn('WARNING!!! node-kafka-streams ktable: flaky implementation of consumeUntilLatestOffset based on fixed debounce (200ms) and timeout (5s)')

        const timeout = 5000
        const waitSinceLast = 200

        super.multicast()
        this.stream$ = this.stream$
            .takeUntil(
                this.stream$.debounce(waitSinceLast)
                .merge(most.periodic(timeout).skip(1))
                .take(1))
