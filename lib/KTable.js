...
class KTable extends StreamDSL {
    ...
    consumeUntilLatestOffset(finishedCallback = null){
        console.warn('WARNING!!! node-kafka-streams ktable: flaky implementation of consumeUntilLatestOffset based on fixed debounce (1s)')

        const waitSinceLast = this.__waitSinceLast || 1000

        super.multicast()
        const stopSignal = most.periodic(waitSinceLast).take(1) // initial debounce/timeout if topic is empty
            .merge(this.stream$)
            .debounce(waitSinceLast)
            .take(1)
